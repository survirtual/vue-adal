...
var AuthenticationContext = function () {
  ...
  function AuthenticationContext(opts) {
    ...
    this.adalContext = opts.adalContext || new adal(this.config);
    ...
    if (this.requireAuthOnInitialize) {
      ...
      if (this.config.endpoints) {
        Object.keys(this.config.endpoints).forEach(function (key, index) {
          var resource = _this.config.endpoints[key];
          _this.acquireToken(resource, function (err, token) {
            if (err) {
              console.log('Could not get token');
            }
          });
  ...
  createClass(AuthenticationContext, [{
    ...
    value: function acquireToken(resource, callback) {
      this.adalContext.acquireToken(resource, function (err, token) {
        if (err) {
          callback(err);
        } else {
          callback(null, token);
    ...
    value: function isAuthenticated() {
      var authenticated = !!this.adalContext.getCachedToken(this.config.clientId);
...
var getToken = function getToken(resource, http, cb) {
  exports.AuthenticationContext.acquireToken(resource, function (err, token) {
    if (err) {
      var errCode = err.split(':')[0];
      switch (errCode) {
        case 'AADSTS50058':
          // Need to prompt for user sign in
          exports.AuthenticationContext.login();
          break;
        case 'AADSTS65001':
          // Token is invalid; grab a new one
          exports.AuthenticationContext.acquireTokenRedirect(resource);
          break;
...
var AxiosAuthHttp$$1 = function () {
  function AxiosAuthHttp$$1() {
    classCallCheck(this, AxiosAuthHttp$$1);
  }

  createClass(AxiosAuthHttp$$1, null, [{
    key: 'createNewClient',
    value: function createNewClient(options) {
      ...
      http.interceptors.response.use(function (response) {
        ...
        if (error.response.status === 401) {
          exports.AuthenticationContext.adalContext.clearCacheForResource(options.resourceId);
      ...
      options.router.beforeEach(function (to, from, next) {
        getToken(options.resourceId, http, function (err, token) {
          ...
          if (options.onTokenSuccess instanceof Function) {
            options.onTokenSuccess(http, exports.AuthenticationContext, token);
          }
          next();
        });
      });
...
var AdalPlugin = {
  install: function install(vue) {
    ...
    vue.prototype.$adal = exports.AuthenticationContext;
...
exports.AxiosAuthHttp = AxiosAuthHttp$$1;
